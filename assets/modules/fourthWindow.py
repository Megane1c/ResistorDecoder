# -*- coding: utf-8 -*-
"""
Created on Sat Sep 29 01:16:49 2018
...
class SubWindow4(tk.Toplevel):
    ...
    ##### POSITION OF GUI #####    
        ...
        self.resizable(0,0)
        self.title("6-Band-Resistor Value Finder")
        self.iconbitmap(default='assets/img/resistor-512.ico')        
        
        
        self.current_colors = ["brown","green","red","royal blue","purple4","orange"] # DEFAULT VALUE
        
        self.create_elements()
           
    ##### CLOSING COMMAND #####        
        self.protocol('WM_DELETE_WINDOW',master.close4) 
        
    
    ##### CREATION OF RESISTOR #####    
    def create_elements(self):
        self.resistor = tk.Canvas(self, width=410, height=100)
        self.resistor.config(bg="white smoke")
        self.resistor.create_rectangle((10,10,408,90), fill="#F3C967")
        ...
        tk.Label(self, text = "Sixth band:\nTemperature Coefficient").grid(row=6, column=0)
        self.band6 = tk.Canvas(self, width=200, height=50)
        self.band6.create_rectangle((0,0,50,50),fill="brown", outline="brown")
        self.band6.create_rectangle((34,0,100,50),fill="red", outline="red")
        self.band6.create_rectangle((68,0,150,50),fill="orange", outline="orange")
        self.band6.create_rectangle((102,0,150,50),fill="yellow", outline="yellow")        
        self.band6.create_rectangle((136,0,250,50),fill="royal blue", outline="royal blue")
        self.band6.create_rectangle((170,0,300,50),fill="purple4", outline="purple4")                      
        self.band6.grid(row=6,column=1, columnspan = 2)
        self.band6.bind("<Button 1>", self.b6)

        self.result = tk.Text(self, width=43, height=1)
        self.result.grid(row=7,column=0, columnspan=3)        

        self.min= tk.Text(self, width=43, height=1)
        self.min.grid(row=8,column=0,columnspan=3,padx=10, pady=10)
        
        self.max= tk.Text(self, width=43, height=1)
        self.max.grid(row=9,column=0,columnspan=3,padx=5, pady=5) 
    ...
    def update(self):
        ...
        self.result.insert(tk.END,"Value: "+str(self.calculate())+"Ω "+"±"+self.ch()+"% "+self.ch1()+" ppm/K")


        self.min.delete(0.0,tk.END)
        self.min.insert(tk.END,"Minimum Value: "+str(self.Min())+"Ω "+self.ch1()+" ppm/K")

        
        self.max.delete(0.0,tk.END)
        self.max.insert(tk.END,"Maximum Value: "+str(self.Max())+"Ω "+self.ch1()+" ppm/K")
    ...
    def ch1(self):
        band = str(self.current_colors[5])
        if band == 'brown':
            return '100'
        elif band == 'red':
            return '50'
